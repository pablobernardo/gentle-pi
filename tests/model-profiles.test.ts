...
test("listModelProfiles filters unsafe names, non-files, and sorts safe files", async (t) => {
	...
	writeJson(join(dir, "beta.json"), {
		kind: "gentle-pi.agent_model_routing",
		version: 1,
		agents: {},
	});
	writeJson(join(dir, "broken.json"), { kind: "x", version: 1, agents: {} });
...
test("readModelProfile rejects symlinked profile files", async (t) => {
	const home = withTempHome(t);
	const outside = mkdtempSync(join(tmpdir(), "gentle-pi-outside-"));
	t.after(() => rmSync(outside, { recursive: true, force: true }));
	...
	const dir = join(home, "model-profiles");
	mkdirSync(dir, { recursive: true });
	writeJson(join(outside, "daily.json"), {
		kind: "gentle-pi.agent_model_routing",
		version: 1,
		agents: { "sdd-apply": { model: "outside/provider" } },
	});
	try {
		symlinkSync(join(outside, "daily.json"), join(dir, "daily.json"));
	} catch {
		return;
	}
...
test("writeModelProfile rejects a symlink instead of clobbering its target", async (t) => {
	const home = withTempHome(t);
	const outside = mkdtempSync(join(tmpdir(), "gentle-pi-outside-"));
	t.after(() => rmSync(outside, { recursive: true, force: true }));
	...
	try {
		symlinkSync(target, join(dir, "daily.json"));
	} catch {
		return;
	}

	await assert.rejects(() =>
		writeModelProfile("daily", {
			"sdd-apply": { model: "profile/provider", thinking: "low" as const },
		}),
	);
	assert.equal(await readFile(target, "utf8"), "keep me");
});

test("readModelProfile rejects oversized profile files", async (t) => {
	const home = withTempHome(t);
	const dir = join(home, "model-profiles");
	mkdirSync(dir, { recursive: true });
	writeFileSync(join(dir, "huge.json"), "{" + " ".repeat(MAX_MODEL_PROFILE_BYTES + 1) + "}");

	assert.equal(await readModelProfile("huge.json"), undefined);
	assert.deepEqual(await listModelProfiles(), []);
});

test("overwriteModelProfile rejects unsafe filenames", async (t) => {
	withTempHome(t);
	await assert.rejects(
		() => overwriteModelProfile("../outside.json", {}),
...
test("writeModelProfile rejects existing profiles and overwriteModelProfile replaces explicitly", async (t) => {
	withTempHome(t);
	const path = await writeModelProfile("daily", {
		"sdd-design": {
			model: "anthropic/claude-sonnet-4",
	...
	await overwriteModelProfile("daily.json", {
		"sdd-design": {
			model: "anthropic/claude-sonnet-4",
			thinking: "low" as const,
		},
...
test("deleteModelProfile removes a safe file and no-ops on missing/unsafe names", async (t) => {
	...
	assert.deepEqual(listAfterUnsafe, []);
	assert.equal(existsSync(join(dir, "broken.json")), true);
	...
	assert.deepEqual(listAfterMissing, []);
});
