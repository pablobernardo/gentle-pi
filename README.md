...
Press `p` to open the named model profile picker. Profiles are JSON files under `~/.pi/gentle-ai/model-profiles/` with filesystem-safe filenames and use the same versioned export envelope as `x`/`r`. Loading validates that envelope before applying it, and profile actions do not create, rewrite, or delete `~/.pi/gentle-ai/models.export.json`.
